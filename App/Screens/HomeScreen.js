...
const HomeScreen = ({navigation}) => {
  ...
  const [combineMarksCh, setCombineMarksCh] = useState([]);
  const [marksCrHrsArray, setMarksCrHrsArray] = useState([]);
  const [marksSgpPointsArray, setMarksSgpPointsArray] = useState([]);
  const [inputHasData, setInputHasData] = useState(Array(8).fill(false));
  const [finalObject, setFinalObject] = useState({});
  const [message, setMessage] = useState(null);
  const [color, setColor] = useState('#3664dc');
  ...
  const handleCrHoursChange = (index, text) => {
    // Update the credit hours array with the new text value
    const newArray = [...marksCrHrsArray];
    newArray[index + 1] = text;
    setMarksCrHrsArray(newArray);
  };
  ...
  return (
    <SafeAreaView
        ...
        <View style={Style.container}>
          ...
          <View
            ...
            <View style={Style.buttonView}>
              ...
              <TouchableOpacity onPress={calculateSGPA} style={Style.button}>
            ...
            <View style={Style.flexStyleView}>
              <TouchableOpacity
                ...
                <Text style={Style.buttonTextWhite}>Calculate CGPA</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      {/* beautiful loader is shown for few milli-seconds before the screen loads */}
      {showLoader && <Loader />}
    </SafeAreaView>
  );
};
